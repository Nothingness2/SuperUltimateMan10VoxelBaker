...
3. ダイアログ右上の `…` メニューから **「ローカルプラグインからロード」** を選択
...
プラグイン本体を編集する場合、毎回プラグインを Disable→Enable するのは面倒なので、DevTools (`Ctrl+Shift+I`) のコンソールで以下を実行すると 1 コマンドで再読み込みできます：

```js
Plugins.all.find(p => p.id === 'super_ultimate_man10_voxel_baker').reload()
```

一度入力すれば ↑ キーで履歴呼び出しできます。よく使うならショートカット関数を定義：

```js
window.rv = () => Plugins.all.find(p => p.id === 'super_ultimate_man10_voxel_baker').reload();
```
